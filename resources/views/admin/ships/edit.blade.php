        ...
        <form method="POST" action="{{ route('admin.ships.update', $ship) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="3" style="border: 2px solid rgba(26, 58, 92, 0.1); border-radius: 10px; padding: 10px 16px;">{{ old('description', $ship->description) }}</textarea>
                @error('description')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <!-- Images -->
            <div class="mb-3">
                <label class="form-label fw-bold" style="color: #1a3a5c;">Current Images</label>
                @if($ship->images->count() > 0)
                    <div class="row">
                        @foreach($ship->images as $image)
                            <div class="col-md-3 mb-2">
                                <div class="position-relative">
                                    <img src="{{ $image->image_url }}" class="img-fluid rounded" style="height: 100px; object-fit: cover; width: 100%; border: 2px solid rgba(26, 58, 92, 0.1);">
                ...
                @else
                    <p class="text-muted" style="color: #5e6b72;">No images uploaded.</p>
                @endif
                <label class="form-label fw-bold" style="color: #1a3a5c; margin-top: 10px;">Upload New Images (optional)</label>
                <input type="file" name="images[]" class="form-control @error('images') is-invalid @enderror" 
                       multiple accept="image/*" style="border: 2px solid rgba(26, 58, 92, 0.1); border-radius: 10px; padding: 10px 16px;">
                <small class="text-muted" style="color: #5e6b72;">Upload additional images (JPEG, PNG, GIF, WEBP - max 2MB each)</small>
                @error('images')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
