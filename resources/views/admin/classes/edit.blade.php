...
@section('title', 'Edit Ship Class')

@section('content')
<div class="card-naval" style="max-width: 700px; margin: 0 auto;">
    <div class="card-header">
        <i class="bi bi-pencil"></i> Edit Ship Class: {{ $class->name }}
    </div>
    <div class="card-body p-4">
        <form method="POST" action="{{ route('admin.classes.update', $class) }}">
            ...
            <div class="mb-3">
                <label class="form-label fw-bold" style="color: #1a3a5c;">Class Name *</label>
            ...
            <div class="mb-4">
                ...
                <select name="type_id" class="form-select @error('type_id') is-invalid @enderror" required
                    ...
                    @foreach($types as $type)
                        <option value="{{ $type->id }}" {{ old('type_id', $class->type_id) == $type->id ? 'selected' : '' }}>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </select>
