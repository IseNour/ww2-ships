...
<div class="card-naval" style="max-width: 1000px; margin: 0 auto;">
    <div class="card-header">
        <i class="bi bi-pencil"></i> Edit Ship: {{ $ship->name }}
    </div>
    <div class="card-body p-4">
        <form method="POST" action="{{ route('admin.ships.update', $ship) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="class_id" class="form-select @error('class_id') is-invalid @enderror" required
                            ...
                            @foreach($classes as $class)
                                <option value="{{ $class->id }}" {{ old('class_id', $ship->class_id) == $class->id ? 'selected' : '' }}>
       ...
       style="border: 2px solid var(--border-color, rgba(26,58,92,0.1)); border-radius: 10px; padding: 10px 16px;">
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" name="displacement" class="form-control @error('displacement') is-invalid @enderror" 
                               value="{{ old('displacement', $ship->displacement) }}"
                               style="border: 2px solid var(--border-color, rgba(26,58,92,0.1)); border-radius: 10px; padding: 10px 16px;">
                        @error('displacement')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" name="max_speed" class="form-control @error('max_speed') is-invalid @enderror" 
                               value="{{ old('max_speed', $ship->max_speed) }}"
                               style="border: 2px solid var(--border-color, rgba(26,58,92,0.1)); border-radius: 10px; padding: 10px 16px;">
                        @error('max_speed')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="fate" class="form-control @error('fate') is-invalid @enderror" 
                               value="{{ old('fate', $ship->fate) }}" placeholder="e.g., Museum ship, Sunk in 1945"
                               style="border: 2px solid var(--border-color, rgba(26,58,92,0.1)); border-radius: 10px; padding: 10px 16px;">
                        @error('fate')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="3" style="border: 2px solid var(--border-color, rgba(26,58,92,0.1)); border-radius: 10px; padding: 10px 16px;">{{ old('description', $ship->description) }}</textarea>
                @error('description')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-3">
                <div class="form-check">
                    <input type="checkbox" name="is_aircraft_carrier" class="form-check-input" 
                           id="isCarrier" value="1" {{ old('is_aircraft_carrier', $ship->is_aircraft_carrier) ? 'checked' : '' }}>
            ...
            <!-- Images -->
            <div class="mb-3">
                <label class="form-label fw-bold" style="color: var(--text-primary, #1a3a5c);">Current Images</label>
                @if($ship->images->count() > 0)
                    <div class="row">
                        @foreach($ship->images as $image)
                            <div class="col-md-3 mb-2">
                                <div class="position-relative">
                                    <img src="{{ $image->image_url }}" class="img-fluid rounded" style="height: 100px; object-fit: cover; width: 100%; border: 2px solid var(--border-color, rgba(26,58,92,0.1));">
                                    ...
                                    @if($image->caption)
                                        <small class="text-muted" style="color: var(--text-secondary, #5e6b72);">{{ $image->caption }}</small>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                ...
                @else
                    <p class="text-muted" style="color: var(--text-secondary, #5e6b72);">No images uploaded.</p>
                @endif
                <label class="form-label fw-bold" style="color: var(--text-primary, #1a3a5c); margin-top: 10px;">Upload New Images (optional)</label>
                ...
                <small class="text-muted" style="color: var(--text-secondary, #5e6b72);">Upload additional images (JPEG, PNG, GIF, WEBP - max 2MB each)</small>
                @error('images')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div id="aircraftSection" style="{{ $ship->is_aircraft_carrier ? 'display: block;' : 'display: none;' }}">
                <div class="mb-3">
                    <label class="form-label fw-bold" style="color: var(--text-primary, #1a3a5c);">
                    ...
                    <div style="max-height: 300px; overflow-y: auto; border: 2px solid var(--border-color, rgba(26,58,92,0.1)); border-radius: 10px; padding: 16px;">
                        @foreach($aircraftModels as $aircraft)
                            @php
                                $pivot = $ship->aircraftModels->where('id', $aircraft->id)->first();
                            @endphp
            ...
            <div class="d-flex gap-3 mt-4">
                <button type="submit" class="btn btn-naval">
                    <i class="bi bi-save"></i> Update Ship
                </button>
