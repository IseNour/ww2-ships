        ...
        <form method="POST" action="{{ route('admin.battles.update', $battle) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                          rows="4" style="border: 2px solid rgba(26, 58, 92, 0.1); border-radius: 10px; padding: 10px 16px;">{{ old('description', $battle->description) }}</textarea>
                @error('description')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-3">
                <label class="form-label fw-bold" style="color: #1a3a5c;">Current Image</label>
                @if($battle->image_url)
                    <div class="mb-2">
                        <img src="{{ $battle->image_url }}" width="200" style="border-radius: 10px; border: 2px solid rgba(26, 58, 92, 0.1);">
                    </div>
                @else
                    <p class="text-muted" style="color: #5e6b72;">No image uploaded</p>
                @endif
                <label class="form-label fw-bold" style="color: #1a3a5c;">Upload New Image (optional)</label>
                <input type="file" name="image" class="form-control @error('image') is-invalid @enderror" 
                       accept="image/*" style="border: 2px solid rgba(26, 58, 92, 0.1); border-radius: 10px; padding: 10px 16px;">
                <small class="text-muted" style="color: #5e6b72;">Upload a new image to replace the current one (JPEG, PNG, GIF, WEBP - max 2MB)</small>
                @error('image')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
                    ...
                    @foreach($ships as $ship)
                        ...
                        <div class="form-check mb-2">
                            ...
                            <input type="text" name="results[]" class="form-control form-control-sm mt-1 result-input" 
                                   ...
                                   style="{{ $pivot ? 'display: block;' : 'display: none;' }} width: 200px; border: 2px solid rgba(26, 58, 92, 0.1); border-radius: 8px; padding: 4px 10px;">
                        </div>
                    @endforeach
                </div>
            </div>
